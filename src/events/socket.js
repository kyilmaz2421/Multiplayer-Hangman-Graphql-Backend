...
const socketConfig = async (socket, io) => {
  ...
  socket.on("send-invitation", async ({ invitation }, callback) => {
    ...
    if (
      invitation.newUser !== invitation.host &&
      (await User.findOne({ username: invitation.newUser })) &&
      !(
        (await Game.find({ isComplete: false, members: invitation.newUser })
          .length) == 0
      )
    ) {
      ...
      socket.broadcast
        .to(invitation.newUser)
        .emit("receive-invite", { invitation });
      console.log("game invite: Succeeded");
    } else {
      console.log("game invite: Failed");
    }
  });
  ...
  socket.on("register-move", async (moveData) => {
    ...
    const status = checkWin(
      moveData.correctLetters,
      moveData.wrongLetters,
      game.word
    );
    if (status == "win") {
      game.win = true;
  ...
  socket.on("register-move-log", async ({ host, playerMoves, playerNames }) => {
    io.in("lobby:" + host).emit("game-update-logs", {
      playerMoves,
      playerNames,
    });
  });
